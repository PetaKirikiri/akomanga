...
type StudentDashboardRow = StudentRow & {
...
type StudentSortKey = 'organization' | 'name' | 'classes' | 'progress' | 'updated';
...
export default function AdminStudentsPage() {
  ...
  const [sortKey, setSortKey] = useState<StudentSortKey>('organization');
  const isSuper = appUser?.role === 'super_admin';
  const navTabs = portalNavForRole(appUser?.role, { adminIfStaff: true });

  const studentsQuery = useQuery({
    queryKey: ['portal', 'admin-student-dashboard'],
    enabled: isSuper,
    queryFn: async () => {
      const [{ data: students, error: studentsError }, { data: enrollments, error: enrollmentsError }, { data: progress, error: progressError }] =
        await Promise.all([
  ...
  const deleteStudent = useMutation({
    mutationFn: async (student: StudentDashboardRow) => {
      ...
      const [{ error: progressError }, { error: enrollmentsError }] = await Promise.all([
        supabase.from('student_capability_progress').delete().eq('student_id', student.id),
        supabase.from('class_enrollments').delete().eq('student_id', student.id),
      ]);
      if (progressError) throw progressError;
      if (enrollmentsError) throw enrollmentsError;
    ...
    onSuccess: async (_data, student) => {
      await qc.invalidateQueries({ queryKey: ['portal', 'admin-student-dashboard'] });
    },
  });
    ...
    >
      <section className="portal-flush-section">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-portal-ink">Student dashboard</h2>
            ...
            <table className="w-full min-w-[56rem] border-collapse text-left text-sm text-portal-ink">
              ...
              <tbody>
                {sortedStudents.map((student) => (
                  <tr key={student.id} className="border-b border-portal-border/70 hover:bg-portal-bg">
