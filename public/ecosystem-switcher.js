/**
 * Same-origin links between Akomanga, Mata, Maumahara, and Pānui.
 * Copy into each satellite repo `public/` and add:
 *   <script defer src="%BASE%ecosystem-switcher.js"></script>
 * where %BASE% is '' for Akomanga, or '/mata/', '/maumahara/', '/panui/' with base-path builds.
 *
 * Visible when hostname is localhost / 127.0.0.1 or localStorage.setItem('ecosystemSwitcher','1').
 */
(function () {
  ...
  for (var i = 0; i < apps.length; i++) {
    ...
    if (!active) {
      link.onmouseenter = function (el) {
        return function () {
          el.style.background = '#fafaf9';
        };
      }(link);
      link.onmouseleave = function (el) {
        return function () {
